...
from .config import registrar, settable
...
def stretchFactors(avail: float, widths: Sequence[float],
...
def stretchRect(orient: Qt.Orientation, item: QtWidgets.QGraphicsWidget,
    ...
    if orient == Qt.Horizontal:
        ...
        if item.sizePolicy().hasHeightForWidth():
            hint = item.effectiveSizeHint(Qt.PreferredSize,
                                          QtCore.QSizeF(w, -1))
            h = hint.height()
    else:
...
def justifyAndAlign(orient: Qt.Orientation, item_rects: Sequence[ItemRectPair],
    ...
    if horiz:
        breadths = [r.width() for _, r in item_rects]
    else:
        breadths = [r.height() for _, r in item_rects]
    total_breadth = sum(breadths) + spacing * (len(item_rects) - 1)

    scale_factors = [1.0] * len(item_rects)
    extra = avail - total_breadth
    offset = 0.0
    if extra >= 1.0 and justify != Justify.even:
        if justify == Justify.stretch:
            scale_factors = stretchFactors(avail, breadths, stretches)
...
class Arrangement(QtCore.QObject):
    ...
    def sizeHint(self, which: Qt.SizeHint, constraint: QtCore.QSizeF,
        ...
        if constraint.width() >= 0:
            maxb = 0.0
            avail = QtCore.QRectF(QtCore.QPointF(0, 0), constraint)
            for _, rect in self.rects(which, avail, items):
                maxb = max(maxb, rect.bottom())
            size = QtCore.QSizeF(constraint.width(), maxb)
        elif constraint.height() >= 0:
            maxr = 0.0
            avail = QtCore.QRectF(QtCore.QPointF(0, 0), constraint)
            for _, rect in self.rects(which, avail, items):
                maxr = max(maxr, rect.right())
            size = QtCore.QSizeF(maxr, constraint.height())
...
class LinearArrangement(OrientedArrangement):
    ...
    def _implicitSizeHint(self, items: Sequence[QtWidgets.QWidget]
        ...
        sizes = [it.effectiveSizeHint(Qt.PreferredSize) for it in items]
...
class FlowArrangement(OrientedArrangement):
    ...
    def _implicitSizeHint(self, items: Sequence[QtWidgets.QWidget]
        ...
        sizes = [it.effectiveSizeHint(Qt.PreferredSize) for it in items]
        total = sum((sz.width() if horiz else sz.height()) for sz in sizes)
        avg = total / len(items)
        side = avg * math.ceil(math.sqrt(len(items)))
        if horiz:
            constraint = QtCore.QSizeF(side, -1)
        else:
            constraint = QtCore.QSizeF(-1, side)
        hint = self.sizeHint(Qt.PreferredSize, constraint, items)
        return hint
...
class ArrangementLayout(QtWidgets.QGraphicsLayout):
    ...
    def itemAt(self, i: int) -> QtWidgets.QGraphicsLayoutItem:
    ...
    def sizeHint(self, which: Qt.SizeHint,
        ...
        return self._arrangement.sizeHint(which, constraint, self._items)


# class OrientedLayout(GraphicLayout):
#     def __init__(self, parent: QtWidgets.QGraphicsItem = None):
#         super().__init__(parent)
...
#         self.invalidate()
#
#     def _updateSizePolicy(self) -> None:
#         policy = self.sizePolicy()
#         orient = self.orientation()
#         policy.setHeightForWidth(orient == Qt.Horizontal)
#         policy.setWidthForHeight(orient == Qt.Vertical)
#         self.setSizePolicy(policy)
#
#
# @layouttype("packed")
# class PackedGraphicsLayout(OrientedLayout):
#     def __init__(self, parent: QtWidgets.QGraphicsItem = None):
#         super().__init__(parent)
...
#         self.invalidate()
#
#     def layoutChildren(self, geom: QtCore.QRectF) -> Iterable[ItemRectPair]:
#         orient = self.orientation()
#         spacing = self.spacing()
#         dense = self.isDense()
#
#         item_rects = self._itemRects(geom)
#         if not item_rects:
#             return
#
#         if self.orientation() == Qt.Horizontal:
...
#
#
# @layouttype("packed_grid")
# class PackedGridGraphicsLayout(PackedGraphicsLayout):
#     def __init__(self, parent: QtWidgets.QGraphicsItem = None):
#         super().__init__(parent)
#         self._cellsize = QtCore.QSizeF(DEFAULT_GRID_COL_WIDTH,
#                                        DEFAULT_GRID_ROW_HEIGHT)
...
#
# @layouttype("flow")
# class FlowGraphicsLayout(OrientedLayout):
#     def __init__(self, parent: QtWidgets.QGraphicsItem = None):
#         super().__init__(parent)
#         self._line_spacing = 5.0
#         self._item_spacing = 5.0
#         self._justify = Justify.start
#         self._content_justify = Justify.start
#         self._align_items = Align.start
#         self._stretches: dict[int, int] = {}
#
#     def setStretchFactor(self, item: QtWidgets.QGraphicsItem, factor: int):
#         self._stretches[id(item)] = factor
#
#     def lineSpacing(self) -> float:
#         return self._line_spacing
#
#     @settable()
#     def setLineSpacing(self, spacing: float):
...
#
#     @settable()
#     def setItemSpacing(self, spacing: float):
...
#
#     def itemAlignment(self) -> Align:
#         return self._align_items
#
#     def setItemAlignment(self, align: Align):
#         self._align_items = align
#         self.invalidate()
#
#     def justification(self) -> Justify:
#         return self._justify
#
#     def setJustification(self, justify: Justify):
#         self._justify = justify
#         self.invalidate()
#
#     def contentJustification(self) -> Justify:
#         return self._content_justify
#
#     def setContentJustification(self, justify: Justify):
#         self._content_justify = justify
#         self.invalidate()
#
#     @staticmethod
#     def _wrapLines(extents: Sequence[float], avail: float) -> Iterable[int]:
#         x = 0.0
#         brk = 0
...
#         orient = self.orientation()
#         line_just = self.justification()
#         item_align = self.itemAlignment()
#         content_just = self.contentJustification()
#         line_spacing = self.lineSpacing()
#         item_spacing = self.itemSpacing()
#         horiz = orient == Qt.Horizontal
#
#         item_rects = self._itemRects()
#         if horiz:
#             extents = [r.width() + item_spacing for _, r in item_rects]
#             avail = geom.width()
#             avail_length = geom.height()
#         else:
#             extents = [r.height() + item_spacing for _, r in item_rects]
#             avail = geom.height()
#             avail_length = geom.width()
#
#         # Break the full list into lines
...
#         line_sizes: list[float] = []
#         i = 0
#         for line_break in self._wrapLines(extents, avail + item_spacing):
#             line = item_rects[i:line_break]
#             lines.append(line)
#             if horiz:
#                 line_size = max(r.height() for _, r in line)
#             else:
#                 line_size = max(r.width() for _, r in line)
...
#             line_sizes, avail_length, line_spacing, content_just
#         )
#         for line, line_size, line_scale in zip(lines, line_sizes, line_scales):
#             positionItemRects(
#                 avail, line, orient, item_spacing, line_just, item_align,
#                 line_scale=line_scale, line_offset=line_offset
#             )
#             line_offset += line_size * line_scale + line_spacing
#         return item_rects
#
#
# @layouttype("switch")
# class SwitchGraphicsLayout(GraphicLayout):
#     def __init__(self, parent: QtWidgets.QGraphicsItem = None):
#         super().__init__(parent)
#         self._hspace = 10.0
#         self._vspace = 0.0
...
#         self._vspace = vspace
#
#     def _stats(self, which: Qt.SizeHint = Qt.PreferredSize
...
#         sum_height = 0.0
#         for item in self.iterItems():
#             isize = item.effectiveSizeHint(which)
#             max_width = max(max_width, isize.width())
#             max_height = max(max_height, isize.height())
#             sum_width += isize.width()
#             sum_height += isize.height()
#         sum_width += (count - 1) * self.horizontalSpacing()
#         sum_height += (count - 1) * self.verticalSpacing()
#         return max_width, max_height, sum_width, sum_height
#
#     def childRects(self, geom: QtCore.QRectF) -> Sequence[QtCore.QRectF]:
#         max_width, max_height, sum_width, sum_height = self._stats()
#         count = self.count()
#         x = geom.x()
#         y = geom.y()
#         hspace = self.horizontalSpacing()
...
#         hpad = (count - 1) * hspace
#         vpad = (count - 1) * vspace
#         horiz = sum_width <= geom.width()
#
#         item_rects: list[QtCore.QRectF] = []
#         stretches: list[int] = []
#         for item in self.iterItems():
#             if not item.isVisible():
#                 continue
#             rsize = item.effectiveSizeHint(Qt.PreferredSize)
#             item_rects.append(QtCore.QRectF(QtCore.QPointF(0, 0), rsize))
#             stretches.append(self._stretches.get(id(item), 1))
#
#         if not item_rects:
#             return []
#
...
#         y = geom.y()
#         if horiz:
#             avail = geom.width() - hpad
#             width = avail / self.count()
#             widths = [r.width() or width for r in item_rects]
#             factors = stretchFactors(avail, widths, stretches)
#             for i, rect in enumerate(item_rects):
#                 width = widths[i] * factors[i]
#                 size = self.itemAt(i).effectiveSizeHint(
#                     Qt.PreferredSize, QtCore.QSizeF(width, -1)
#                 )
#                 rect.setSize(size)
#                 rect.setHeight(max(rect.height(), geom.height()))
#                 rect.moveTo(x, y)
#                 x += rect.width() + hspace
#         else:
#             for rect in item_rects:
#                 rect.setRect(x, y, geom.width(), rect.height())
#                 y += rect.height() + vspace
#         return item_rects
#
#
# @layouttype("matrix")
# class MatrixGraphicsLayout(GraphicLayout):
#     def __init__(self, parent: QtWidgets.QGraphicsItem = None):
#         super().__init__(parent)
#         self._matrix = Matrix()
#
...
#
#     # def orientation(self) -> Qt.Orientation:
#     #     return self._matrix.orientation()
#     #
#     # @settable(argtype=Qt.Orientation)
#     # def setOrientation(self, orient: Qt.Orientation):
#     #     self._matrix.setOrientation(orient)
#     #     self.invalidate()
#
#     def maximumColumnCount(self) -> int:
#         return self._matrix.maximumColumnCount()
#
#     @settable("max_column_count")
#     def setMaximumColumnCount(self, count: int) -> None:
#         self._matrix.setMaximumColumnCount(count)
#         self.invalidate()
#
#     def horizontalSpacing(self) -> float:
#         return self._matrix.horizontalSpacing()
#
#     @settable("h_spacing")
#     def setHorizontalSpacing(self, hspace: float):
#         self._matrix.setHorizontalSpacing(hspace)
#         self.invalidate()
#
#     def verticalSpacing(self) -> float:
#         return self._matrix.verticalSpacing()
#
#     @settable("v_spacing")
#     def setVerticalSpacing(self, vspace: float):
#         self._matrix.setVerticalSpacing(vspace)
#         self.invalidate()
#
#     @settable("spacing")
#     def setSpacing(self, spacing: float):
#         self._matrix.setSpacing(spacing)
#         self.invalidate()
#
#     def minimumColumnWidth(self) -> float:
#         return self._matrix.minimumColumnWidth()
#
#     @settable("min_column_width")
#     def setMinimumColumnWidth(self, width: float):
#         self._matrix.setMinimumColumnWidth(width)
#         self.invalidate()
#
#     def maximumColumnWidth(self) -> float:
#         return self._matrix.maximumColumnWidth()
#
#     @settable("max_column_width")
#     def setMaximumColumnWidth(self, width: float):
#         self._matrix.setMaximumColumnWidth(width)
#         self.invalidate()
#
#     def rowHeight(self) -> float:
#         return self._matrix.rowHeight()
#
#     @settable()
#     def setRowHeight(self, height: float):
#         self._matrix.setRowHeight(height)
#
#     def layoutChildren(self, geom: QtCore.QRectF) -> Iterable[ItemRectPair]:
#         matrix = self._matrix
#         width = geom.width()
#         vis_items = [it for it in self.iterItems() if it.isVisible()]
#         count = len(vis_items)
#
#         i = 0
#         for item in self.iterItems():
...
#         # maxwidth = self.maximumColumnWidth()
#         # h_space = self.horizontalSpacing()
#         # v_space = self.verticalSpacing()
#         # items = [it for it in self.iterItems() if it.isVisible()]
#         # if not items:
#         #     return
#         #
#         # row_height = self.rowHeight()
#         # if row_height < 0:
#         #     _, _, colwidth = matrixSizes(geom.width(), len(items), orient,
#         #                                  minwidth, h_space)
#         #     constraint = QtCore.QSizeF(colwidth, -1)
#         #     row_height = max(
#         #         it.effectiveSizeHint(Qt.PreferredSize, constraint).height()
#         #         for it in items
#         #     )
#         #
#         # rects = cellRects(
#         #     geom.width(), len(items), orient, minwidth, row_height,
#         #     start_x=geom.x(), start_y=geom.y(),
#         #     h_space=h_space, v_space=v_space, max_width=maxwidth
#         # )
